...
export class ElasticCollection implements IElasticCollection {
    ...
    uniqueId():string { return [ this.network, 'collection', this.collection_name].join('-') }
    collection_name: string;
    name: string;
    img: string;
    description: string;
...
export class ElasticTemplate implements IElasticTemplate {
    constructor(o:any, network:'wax' | 'eos' | 'proton') {
        ...
        this.is_transferable = o.is_transferable
        this.is_burnable = o.is_burnable
    ...
    uniqueId():string { return [ this.network, 'template', this.template_id].join('-') }
    decodeDescription(o:any) : string {
        if ( o.schema && o.schema.format && o.data ) {
            ...
            if ( o.collection.name ) { return o.collection.name }
        }
        return ''
    }
    template_id: string;
...
export class ElasticSchema implements IElasticSchema {
    constructor(o:any, network:'wax' | 'eos' | 'proton') {
        ...
        this.author = o.collection.author
        this.contract = o.contract
        this.network = network
        this.created_at_time = o.created_at_time
        this.created_at_block = o.created_at_block
    }
    uniqueId():string { return [ this.network, 'schema', this.collection_name, this.schema_name].join('-') }
    schema_name: string;
    collection_name: string;
    description: string;
    format: any[];
